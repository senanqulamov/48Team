  ...
  return (
    <section aria-labelledby="gallery-title" className="bg-background/60 rounded-2xl shadow p-5 md:p-6 border border-primary/10">
      ...
      <AnimatePresence>
        {lightbox.open && (
          <motion.div
            className="fixed inset-0 z-[1005] flex items-center justify-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            role="dialog"
          ...
          >
            <div
              ...
              aria-hidden="true"
            />
            <div className="relative z-[1006] w-full h-full flex items-center justify-center">
              ...
              {(images[lightbox.index].caption || images[lightbox.index].alt) && (
                <div className="absolute bottom-0 left-0 right-0 mx-auto max-w-3xl text-center text-sm md:text-base text-white/95 bg-black/40 px-4 py-3">
                  {images[lightbox.index].caption ?? images[lightbox.index].alt}
                </div>
              )}
              ...
              <div className="absolute inset-x-0 top-0 flex justify-end p-4">
                <button
                  type="button"
                  className="inline-flex items-center justify-center w-11 h-11 rounded-full bg-black/60 text-white hover:bg-black/70 focus:outline-none focus:ring-2 focus:ring-primary/40 cursor-pointer"
                  onClick={closeLightbox}
